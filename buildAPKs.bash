...
_INPKGS_() {
	...
	then 
		...
		yes | apt install "${PKGS[@]}" || printf "\\e[1;37;5;116m%s\\e[0m\\n" "$STRING2"
	fi
}
...
if [[ ! -d "$RDR" ]]
then
	cd && git clone --depth 1 git://github.com/OrangeX4/buildAPKs --branch master --single-branch || printf "\\e[1;38;5;117m%s\\e[0m\\n" "$STRING2"
fi
